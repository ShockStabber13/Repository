...
const localAuthEnabled = process.env.HACKERAI_LOCAL_AUTH === "1";
const localAuthIssuer = process.env.HACKERAI_LOCAL_AUTH_ISSUER ?? "";
...
function createLocalProvider() {
  const issuer = localAuthIssuer.trim();
  if (!issuer) {
    throw new Error(
      "HACKERAI_LOCAL_AUTH_ISSUER is required when HACKERAI_LOCAL_AUTH=1",
    );
  }
  return {
    type: "customJwt" as const,
