...
import { featherlessDynamicFetch } from "@/lib/ai/featherless-dynamic-fetch";
...
export const MALFORMED_PDF_USER_RESPONSE =
  "This attachment isnâ€™t a valid PDF; re-export or re-upload it.";
...
/* HACKERAI_FEATHERLESS_PATCH_V1_1
 * Provider-only integration.
 * HackerAI's agent loop, local sandbox, terminal tools, file tools,
 * retries, task state, and durable runtime are intentionally untouched.
 */
/* HACKERAI_FEATHERLESS_DYNAMIC_SETTINGS_V1
 * API key and selected model are resolved at request time from the local,
 * git-ignored Featherless settings file.
 */
const featherless = createOpenAI({
  apiKey: "hackerai-local-settings",
  baseURL:
    process.env.FEATHERLESS_BASE_URL?.trim() ||
    "https://api.featherless.ai/v1",
  name: "featherless",
  fetch: featherlessDynamicFetch,
});
...
const openrouterProviders = buildProviderMap(openrouter);

const useFeatherless =
  process.env.HACKERAI_AI_PROVIDER?.trim().toLowerCase() === "featherless";

const buildFeatherlessProviderMap = (): typeof openrouterProviders => {
  // This placeholder never reaches Featherless. featherlessDynamicFetch
  // replaces the request body's model field with the current user selection.
  const model = featherless.chat("hackerai-user-selected-model");

  return Object.fromEntries(
    Object.keys(openrouterProviders).map((key) => [key, model]),
  ) as typeof openrouterProviders;
};

const baseProviders = useFeatherless
  ? buildFeatherlessProviderMap()
  : openrouterProviders;
...
export function isAnthropicModel(modelName: string): boolean {
  if (useFeatherless) return false;
