...
export function useAuthFromAuthKit(
  ...
  const localAuthEnabled =
    process.env.NEXT_PUBLIC_HACKERAI_LOCAL_AUTH === "1";
  const localAuthUrl =
    process.env.NEXT_PUBLIC_HACKERAI_LOCAL_AUTH_URL ??
    "http://127.0.0.1:3211";
  const fetchLocalAccessToken = useCallback(async (): Promise<string | null> => {
    try {
      const response = await fetch(`${localAuthUrl}/token`, { cache: "no-store" });
  ...
  if (localAuthEnabled) {
    return {
      ...
      fetchAccessToken: fetchLocalAccessToken,
    };
  }
