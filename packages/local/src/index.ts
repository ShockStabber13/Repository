 ...
 *
 * On this Windows local fork, command execution is fail-closed through the
 * dedicated OpenTerminal-AI WSL2 + Bubblewrap boundary. Only /workspace is
 * exposed from Windows. Direct-host execution requires an explicit override.
 *
 * Usage:
...
import {
  getIsolatedOsInfo,
  getIsolatedSpawnSpec,
  isWslIsolationEnabled,
  makeIsolatedPayload,
  verifyWslIsolation,
  type IsolatedCommandPayload,
} from "./wsl-isolation";

const DEFAULT_SHELL = getDefaultShell(os.platform());
const WSL_ISOLATION_ENABLED = isWslIsolationEnabled();
...
export class LocalSandboxClient {
  ...
  async start(): Promise<void> {
    ...
    if (WSL_ISOLATION_ENABLED) {
      console.log(chalk.blue("🔒 Verifying WSL/Bubblewrap isolation..."));
      verifyWslIsolation();
      console.log(
        chalk.green(
          "✓ Isolated: only B:\\Artificial Intelligence Workspace is writable as /workspace",
        ),
      );
    } else {
      console.log(
        chalk.yellow(
          "⚠️  Unisolated direct-host execution was explicitly enabled.",
        ),
      );
    }
    await this.connect();
  ...
  private getOsInfo(): OsInfo {
    if (WSL_ISOLATION_ENABLED) return getIsolatedOsInfo();
    return {
  ...
  private getCapabilities(): ClientCapabilities {
    return {
      commands: true,
      // PTY messages would otherwise execute through the Windows node-pty
      // process runner. Keep them disabled in fail-closed WSL mode. Normal
      // streamed shell/file commands remain fully available.
      pty: WSL_ISOLATION_ENABLED ? false : isPtyAvailable(),
    };
  }
  ...
  private async handleCommand(msg: CentrifugoCommandMessage): Promise<void> {
    ...
    try {
      if (WSL_ISOLATION_ENABLED) {
        const payload = makeIsolatedPayload({
          command,
          cwd,
          env,
          timeoutMs: timeout,
        ...
        if (background) {
          const pid = await this.spawnBackground(command, payload);
          ...
          console.log(
            chalk.green(`✓ Isolated background process started with PID: ${pid}`),
          );
          return;
        }
  ...
  private async streamCommand(
    ...
    shouldShow: boolean,
    displayText: string,
    isolatedPayload?: IsolatedCommandPayload,
  ): Promise<void> {
    ...
    return new Promise<void>((resolve) => {
      ...
      if (isolatedPayload) {
        const isolated = getIsolatedSpawnSpec();
        proc = spawn(isolated.command, isolated.args, {
          stdio: ["pipe", "pipe", "pipe"],
          windowsHide: true,
        });
        proc.stdin?.end(`${JSON.stringify(isolatedPayload)}\n`);
      } else {
        ...
        proc = spawn(DEFAULT_SHELL.shell, spawnSpec.args, {
  ...
  private async spawnBackground(
    ...
    if (isolatedPayload) {
      ...
      const result = spawnSync(isolated.command, isolated.args, {
        input: `${JSON.stringify(isolatedPayload)}\n`,
        encoding: "utf8",
        stdio: ["pipe", "pipe", "pipe"],
        windowsHide: true,
        timeout: 15_000,
      ...
      if (result.error || result.status !== 0) {
        throw new Error(
          result.error?.message ||
            result.stderr?.trim() ||
            `Isolated background launcher exited with ${result.status}`,
        );
      }
      const pid = Number.parseInt(result.stdout.trim(), 10);
      if (!Number.isFinite(pid)) {
        throw new Error(`Invalid isolated background PID: ${result.stdout}`);
      }
      return pid;
    }
  ...
  private async handlePtyCreate(msg: PtyCreateMessage): Promise<void> {
    ...
    if (WSL_ISOLATION_ENABLED) {
      await this.publishToChannel({
        ...
        sessionId,
        message:
          "Interactive PTY is disabled in fail-closed WSL isolation mode. Use normal terminal commands; they execute inside /workspace.",
      });
      return;
    }
...
${chalk.cyan("Windows isolation:")}
  This local fork defaults to the dedicated OpenTerminal-AI WSL2 + Bubblewrap
  sandbox. Only B:\\Artificial Intelligence Workspace is exposed as /workspace.
  If isolation verification fails, the client refuses to connect.
